...
function MarkTest() {
  ...
  const handleFilter = (data, conditionSearch) => {
    ...
    return data.filter((item) => {
      ...
      if (conditionSearch.expired === 1) {
        matchExpired = item; // giữ tất cả
      } else {
        matchExpired = today.isSameOrAfter(dayjs(item.ngay_het_han)); // chỉ lấy item quá hạn
      }
    ...
    <MainLayout>
      <Spin spinning={isLoadingInit} tip="Đang tải dữ liệu...">
        <HomeWrapper>
          {danhmuc && (
            <div className="bg-white border border-gray-200 rounded-lg p-4 mb-3 shadow-2xs">
              <Flex wrap gap={16} algin="center">
                  ...
                  <Space>
                    {[
                      ...
                      <Button
                        ...
                        style={{
                          background:
                            searchValue.status === ele.id
                              ? ele?.props.color
                              : "white",
                          color:
                            searchValue.status === ele.id ? "white" : "#444",
                          borderColor: searchValue.status === ele.id
                              ? ele?.props.color
                              : "#eee",
                        }}
                ...
                {/* <Flex className="h-full" align="flex-end">
                  <Button
                    type="default"
                    onClick={onSearch}
                    loading={isLoading}
                    icon={<Search style={{ position: "relative" }} />}
                  ></Button>
                </Flex> */}
              </Flex>
            </div>
          ...
          >
            <div>
              ...
              <div className="text-sm italic text-gray-500">
                Công việc sắp xếp theo thứ tự ngày giao gần nhất
              </div>
            </div>
            <Space>
              <Button
                type="default"
                onClick={() => setModalCongViec(true)}
          ...
          <Table
            ...
            rowClassName={(record) =>
              checkIsDealine(record).quahan &&
              !checkIsDealine(record).isComplete
                ? "task-expired"
                : "task-normal"
            }
            pagination={{
              ...
              },
              {
                ...
                render: (value, row) => (
                  <>
                    ...
                    <div>
                      <Typography.Text>
                        {user.username === "phanducanh.nan" && row.lanh_dao_vtt}{" "}
                  ...
                  <>
                    <div>
                      <Typography.Text strong>
                        ...
                        {row.don_vi_chu_tri
                          .map((item) => {
                            return danhmuc?.don_vi_chu_tri.find(
                              (ele) => ele.id === item
                            ).name;
                          })
                          .join(", ")}
                    ...
                    <div>
                      <div className="line-clamp-3">
                        ...
                        {row.don_vi_phoi_hop
                          .map((item) => {
                            return danhmuc?.don_vi_phoi_hop.find(
                              (ele) => ele.id === item
                            ).name;
                          })
                          .join(", ")}
                      ...
                      {!checkIsDealine(row).quahan && !row.status && (
                        <div>
                          <Typography.Text
                            italic
                            style={{
                              color: !checkIsDealine(row).quahan
                                ? "green"
                                : "red",
                            }}
                          >
      ...
      {modalCongViec && (
        <Drawer
          open={modalCongViec}
          width={"40%"}
          onClose={() => setModalCongViec(false)}
        >
          <FormCongViec
            danhmuc={danhmuc}
            onFinish={() => {
